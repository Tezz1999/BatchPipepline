...
def create_emr_cluster():
  cluster_id = client.run_job_flow(
    ...
    'Ec2KeyName': 'airflowServer',
    'KeepJobFlowAliveWhenNoSteps': True,
    'TerminationProtected': False,
    'Ec2SubnetId': 'subnet-0558182b461cf728d',
    },
    LogUri="s3://emrdatalogs/",
...
with dag:
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','s3://us-east-1.elasticmapreduce/libs/script-runner/script-runner.jar',[ 's3://emrdata/scripts/ingest.sh']],
  dag=dag, 
  )
  ...
  op_args=['{{ ti.xcom_pull("create_emr_cluster")["JobFlowId"]}}','command-runner.jar',[ 'spark-submit',
            '--master', 'yarn',
            '--deploy-mode', 'cluster',
            's3://emrdata/scripts/transform.py']],
  dag=dag, 
  )
